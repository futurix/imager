...
begin
  ...
  // initializing COM (as it might not be initialized in command-line tools)
  ...
  else if ((param = '/cleanfix') or (param = '-cleanfix')) then
    begin
    CleanUp();
    CleanFix();
    end
...
begin
  ...
  if reg.OpenKey(sModules, false) then
    begin
    reg.RStrings(sNotRecommended, rare);
    reg.CloseKey();
    end;
  ...
  if reg.OpenKey(sReg + '\Capabilities', true) then
    ...
    reg.WString('ApplicationDescription', Format(FXL_REG_FF_OVERVIEW, [sAppName]));
    reg.CloseKey();
...
begin
  try
    aar := CreateComObject(CLASS_ApplicationAssociationRegistration) as IApplicationAssociationRegistration;
    if Assigned(aar) then
      aar.ClearUserAssociations();
  except
  end;
end;
