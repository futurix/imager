...
var
  ...
  ext: string;
  item: TListItem;
  includeRare, registered: boolean;
begin
  exts := fx.Plugins.ListOpen();
  includeRare := cbxRare.Checked;
  ...
  for ext in exts do
    begin
    registered := GetExt(ext);

    if (registered or includeRare or not fx.Plugins.IsRareFormat(ext)) then
      begin
      item := lvwFormats.Items.Add();
      item.Caption := ext;
      item.SubItems.Add(fx.Plugins.FindFormatDescription(ext));
      item.Checked := registered;
      end;
    end;
