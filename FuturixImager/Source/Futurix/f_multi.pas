...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Dialogs, Controls, Forms,
...
begin
  ...
  infMulti.lib := LoadLibrary(PWideChar(FxRegRStr(ExtractExt(path), '', sModules + '\' + PS_FOPENMULTI)));
  ...
  if (infMulti.lib <> 0) then
    ...
    if ((@infMulti.FxImgMultiStart <> nil) and
      ...
      begin
      // settings
      tmp_res := infMulti.FxImgMultiStart(PWideChar(path), PWideChar(ExtractExt(path)), Application.Handle, frmMain.Handle, FxImgGlobalCallback);
