...
type
  TfrmCustTB = class(TForm)
    ...
    procedure lbxAvailableDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
    ...
    procedure lbxBarDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
...
begin
  if (lbxBar.Count > 0) then
    ...
    // saving
    FxRegWStr('UI_ToolbarButtons', tmp);
    end;
end;
...
begin
  if (lbxBar.ItemIndex <> -1) then
    lbxBar.Items.Insert(lbxBar.ItemIndex, 'Separator')
  else
    lbxBar.Items.Insert(lbxBar.Count - 1, 'Separator');
end;
...
begin
  if ((lbxBar.ItemIndex <> -1) and (lbxBar.Items[lbxBar.ItemIndex] <> 'Separator') and (lbxBar.Items[lbxBar.ItemIndex] <> '')) then
    begin
    lbxAvailable.Items.Add(lbxBar.Items[lbxBar.ItemIndex]);
    lbxBar.Items.Delete(lbxBar.ItemIndex);
    end
  else
    if ((lbxBar.ItemIndex <> -1) and (lbxBar.Items[lbxBar.ItemIndex] = 'Separator')) then
      lbxBar.Items.Delete(lbxBar.ItemIndex);
end;
...
begin
  if ((Source is TListBox) and (TListBox(Source).Name = lbxBar.Name) and (lbxBar.ItemIndex <> -1)) then
    ...
    if (lbxBar.Items.Strings[lbxBar.ItemIndex] <> 'Separator') then
      begin
      if (i = -1) then
