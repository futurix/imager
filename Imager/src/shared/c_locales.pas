...
begin
    ...
    if locale_reg.OpenKey(sReg, false) then
    	begin
        locale_name := locale_reg.RStr('CurrentLocale', '');
        locale_reg.CloseKey();

        if (locale_name <> '') then
        	begin
            if locale_reg.OpenKey(sModules + '\' + PS_FLOCALE, false) then
            	begin
                locale_str := locale_reg.RStr(locale_name, '');
                locale_reg.CloseKey();
