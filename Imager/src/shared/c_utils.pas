...
uses
  Windows, SysUtils, Classes, UxTheme, ShlObj, ActiveX, StdCtrls,
  Controls, Graphics, c_const;
...
var
    ...
    ansi: array[0..MAX_RES_STRING_SIZE] of char;
begin
    ...
    if (inst <> 0) then
        ...
        if (LoadStringW(inst, id, buf, MAX_RES_STRING_SIZE) > 0) then
        	begin
        	if (WideCharToMultiByte(CP_ACP, 0, buf, -1, ansi, MAX_RES_STRING_SIZE, nil, nil) > 0) then
            	Result := ansi;
            end;
        end;
end;
