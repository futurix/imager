...
type
  ...
  { TImageListDrawParams with added CommCtl 6 paramters }

  {$EXTERNALSYM _IMAGELISTDRAWPARAMS2}
  _IMAGELISTDRAWPARAMS2 = packed record
    cbSize: DWORD;
    himl: HIMAGELIST;
    i: Integer;
    hdcDst: HDC;
  ...
  PImageListDrawParams2 = ^TImageListDrawParams2;
  TImageListDrawParams2 = _IMAGELISTDRAWPARAMS2;
...
uses
  SysUtils, Consts, Forms, ActiveX, Themes;
...
var
  DrawParams: TImageListDrawParams2;
begin
  if HandleAllocated then
  begin
    ...
    DrawParams.himl := Handle;
    DrawParams.i := Index;
    DrawParams.hdcDst := Canvas.Handle;
    DrawParams.x := X;
    DrawParams.y := Y;
    DrawParams.cx := 0;
    DrawParams.cy := 0;
    DrawParams.rgbBk := GetRGBColor(BkColor);
    DrawParams.rgbFg := GetRGBColor(BlendColor);
    DrawParams.fStyle := Style;
