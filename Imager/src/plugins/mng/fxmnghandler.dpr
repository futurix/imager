library fxmnghandler;

uses
  FastMM, c_const, c_locales,
...
begin
    ...
    info_call(PT_FNAME, 'MNG/JNG Plug-in', '');

	info_call(PT_FOPEN, 'mng', '');
	info_call(PT_FOPEN, 'jng', '');

	info_call(PT_FOPENANIM, 'mng', '');

	info_call(PT_FDESCR, 'mng', PChar(LoadLStr(1015) + ' (*.mng)'));
	info_call(PT_FDESCR, 'jng', PChar(LoadLStr(1016) + ' (*.jng)'));
end;
...
begin
  ...
  try
    ...
    try
      LoadXNG(document_path);
      DisplayFirstFrameXNG;

      Result.result_value := OFBitmap.ReleaseHandle;
...
begin
  ...
  begin
    ...
    try
      ...
      Result.result_value := OHBitmap.ReleaseHandle;
    finally
      OHBitmap.Free;
    end;
  end;

  Result.result_xtra := IFDelay;
  BFFirst := false;
