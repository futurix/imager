...
for i:=1 to ParamCount() do
  ...
  if FileExists(ParamStr(i)) then
    ...
    if DirExists(ParamStr(i)) then
      ...
      begin
      if ParamStr(i)='/fs' then
        if not infSettings.full_screen then
          ToggleFS();
      if ParamStr(i)='/reg' then
        begin
...
procedure ProcessStartUp();
begin
if soSlideShow in infSettings.showparams then
  begin
  if soDirAsParam in infSettings.showparams then
    begin
    // slide show in dir
    ...
    if files.Count>0 then
      ...
      if infSettings.full_screen then ToggleFS();
    infSettings.dir:='';
    end
  else
    begin
...
if lib<>0 then
  ...
  MessageBox(0,'Required DLL "PLUGSCAN.DLL" not found. Plug-ins cannot be scanned.','Error!',MB_OK+MB_ICONERROR);
end;
