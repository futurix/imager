...
// additional checks
reg.OpenKey(sModules,true);
if reg.ReadString('DefaultFilter')='' then
  reg.WriteString('DefaultFilter','*.*');
if reg.ReadString('BrowsingMask')='' then
  begin
  tmp:='*.bmp;*.gif;*.jpg;*.jpeg;*.ico;*.wmf;*.emf';
  if FileExists(current_folder + '\f_gex.dll') then
    tmp:=tmp + ';*.pcd;*.psd';
  if FileExists(current_folder + '\f_mng.dll') then
    tmp:=tmp + ';*.mng;*.jng';
