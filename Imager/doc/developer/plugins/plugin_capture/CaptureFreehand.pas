...
procedure TCaptureFreehandForm.FormMouseDown ( Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer );
begin
  fDragging := true;
...
procedure TCaptureFreehandForm.FormMouseMove ( Sender: TObject; Shift: TShiftState;
  X, Y: Integer );
begin
  if fdragging then
  begin
...
procedure TCaptureFreehandForm.FormMouseUp ( Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer );
var
  ScreenDC: HDC;
  hrg: thandle;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        ScreenDC := GetDC ( 0 );
        try
          BitBlt ( fCapBMP.Canvas.handle, 0, 0, dxmax - dxmin, dymax - dymin, fBMP.Canvas.Handle, dxmin, dymin, SRCCOPY );
        finally; ReleaseDC ( 0, ScreenDC ); end;
      finally; DeleteObject ( hrg ); end;
      Dispose ( Memdat );
    end;
    fDragging := false;
    ModalResult := mrOK;
