...
begin
    ...
    mview.MIO.LoadFromFileGIF(String(document_path));

    gframes := mview.MIO.ParamsCount;
    gfile := String(document_path);
...
begin
    ...
    bmp.PixelFormat := pf24bit;
    Result.result_value := bmp.ReleaseHandle();
    Result.result_xtra := mview.MIO.Params[gframe].GIF_DelayTime*10;
    FreeAndNil(bmp);

    if (Result.result_xtra < 10) then
		Result.result_xtra := 10;
