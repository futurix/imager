...
begin
  ...
  aDC := GetDC ( 0 );
  // Mouser, 06/18/05 -
  BitBlt (fBMP.Canvas.handle, 0, 0, Screen.Monitors[M].Width, Screen.Monitors[M].Height, aDC,  Screen.Monitors[M].Left, Screen.Monitors[M].Top, srcCopy );
  ReleaseDC ( 0, aDC );
...
procedure TCaptureSpecificRectForm.FormMouseDown ( Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer );
begin
  RemoveTheRect; // Erase any existing rectangle
  X1 := X; Y1 := Y;
  X2 := X1 + CaptureWidth; Y2 := Y1 + CaptureHeight;
  DrawTheRect;
  SetRect ( fRect, X, Y, X2, Y2 ); // Set initial rectangle position
end;

procedure TCaptureSpecificRectForm.FormMouseMove ( Sender: TObject; Shift: TShiftState; X,
  Y: Integer );
begin
  RemoveTheRect; // Erase any existing rectangle
  X1 := X;
...
procedure TCaptureSpecificRectForm.FormMouseUp ( Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer );
var
  ScreenDC: HDC;
  Bitmap: TBitmap;
   ...
   // Exit if improper rectangle drawn
    ...
    begin
      ...
      try
        ...
        BitBlt ( Bitmap.Canvas.Handle, 0, 0, Bitmap.Width, Bitmap.Height, ScreenDC,
                 Screen.Monitors[M].Left + fRect.Left, Screen.Monitors[M].Top + fRect.Top, SRCCOPY );
