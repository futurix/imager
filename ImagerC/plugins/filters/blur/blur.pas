...
var
   ...
   ACol: PRow; P:PRow;
begin
if (theBitmap.HandleType <> bmDIB) or (theBitmap.PixelFormat <> pf24Bit) then raise exception.Create('GBlur может работать только с 24-битными изображениями');
...
for Row:= 0 to theBitmap.Height - 1 do BlurRow(Slice(theRows[Row]^, theBitmap.Width), K, P);
...
begin
canceled:=false;
if trbAmount.Position=0 then canceled:=true
else GBlur(bmp,(trbAmount.Position/10));
Close();
end;
