...
uses globals, f_reg, f_debug, f_ui, f_strutils, f_tools,
...
reg.OpenKey(sModules + '\Multi',true);
infMulti.lib:=LoadLibrary(PChar(GetAppFolder() + 'plugins\' + reg.RStr(GetExt(path),'')));
reg.CloseKey();
...
if infMulti.lib<>0 then
  ...
  if ((@infMulti.FIPISmultiInit<>nil) and
    ...
    // settings
    if infMulti.FIPISmultiInit(PChar(path),PChar(GetExt(path)),Application.Handle)=0 then
      begin
      CloseMulti();
      FileNotFound(path);
      Exit;
