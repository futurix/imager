...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Dialogs, Forms, IniFiles, athread, c_const;
...
uses globals, f_reg, f_debug, f_graphics, f_tools, f_ui,
...
if reg.RStr(ext,'')<>'' then Result:=true
  else Result:=false;
reg.CloseKey();
end;
...
reg.OpenKey(sModules + '\Anim',true);
infAnim.lib:=LoadLibrary(PChar(GetAppFolder() + 'plugins\' + reg.RStr(GetExt(path),'')));
reg.CloseKey();
if infAnim.lib<>0 then
  ...
  if ((@infAnim.FIPISanimInit<>nil) and
    ...
    begin
    // settings
    if infAnim.FIPISanimInit(PChar(path),PChar(GetExt(path)),Application.Handle)=0 then
      begin
      CloseAnim();
      FileNotFound(path);
      Exit;
    ...
    if same_folder=false then ScanFolder(path)
      else FindFileNumber();
