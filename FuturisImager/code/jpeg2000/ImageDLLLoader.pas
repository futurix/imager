...
type
  ...
  TImageDLLLoader = class(TBitmapLoader)
    ...
    public
      ...
      function FindDLLs(lib: string): Boolean;
...
uses DelphiStream, PluginLinearBitmap;
...
function TImageDLLLoader.FindDLLs(lib: string): Boolean;
var
  Ext, Filter : string;
...
Loader[L].Handle:=LoadLibrary(PChar(lib));
...
if Loader[L].Handle<>0 then
  ...
  if (Assigned(@Loader[L].LoadImage) or Assigned(@Loader[L].SaveImage)) and Assigned(@Loader[L].FreeImage) then
    ...
    if Assigned(@GetFilter) then
      ...
      Loader[L].LoadFormats:='';

    GetFilter:=GetProcAddress(Loader[L].Handle,'?LibGetSaveFilter@@YGPADXZ');
    if Assigned(@GetFilter) then
      Loader[L].SaveFormats:=GetFilter
    else
      Loader[L].SaveFormats:='';
