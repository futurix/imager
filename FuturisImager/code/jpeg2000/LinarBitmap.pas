...
type
  ...
  TLinearBitmap = class(TAssignObject)
    ...
    public
      ...
      constructor Create(Other: TLinearBitmap); overload; // Create copy
      ...
      procedure LoadFromStream(Stream: TStream; const FormatExt: string); overload;
      procedure SaveToStream(Stream: TSeekableStream; const FormatExt: string); overload;
      procedure SaveToStream(Stream: TStream; const FormatExt: string); overload;
      ...
      function MakeDIB(var Bitmap: PBitmapInfo): Integer;
      ...
      // Paint to TBitmap (target size and pelette assumed correct)
  ...
  TBitmapLoader = class(TMonitorObject)
    public
      function CanLoad(const Ext: string): Boolean; virtual;
      function CanSave(const Ext: string): Boolean; virtual;
...
function MakeHPalette(var Pal: TPalette; ColorCount: Integer = 256): HPALETTE;
...
constructor TLinearBitmap.Create(Other: TLinearBitmap);
begin
  inherited Create;
  fPixelFormat:=pfCustom;
  Assign(Other);
...
begin
  ...
  begin
    ...
    begin
      ...
      Move(TLinearBitmap(Other).Map^,Map^,Size);
      if PixelFormat=pf8bit then Palette^:=TLinearBitmap(Other).Palette^;
    end
    else TLinearBitmap(Other).Dispose;
  end
...
begin
  ...
  else Clear(((Color and $ff)*2+((Color shr 8) and $ff)*3+((Color shr 16) and $ff)) div 6);
end;
...
function TLinearBitmap.MakeDIB(var Bitmap: PBitmapInfo): Integer;
var
 Pix : ^Byte;
 LinLen, Y : Integer;
begin
...
begin
  ...
  CopySize:=DestRect.Right-DestRect.Left;
  if PixelFormat=pf24bit then CopySize:=CopySize*3;
